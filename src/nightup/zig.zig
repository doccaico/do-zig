...
pub fn run(dist_dir: []const u8, download_dir: []const u8) !u8 {
    ...
    // 8. ワークスペースを作業パスから dist_dir へ移動
    Io.Dir.rename(Io.Dir.cwd(), work_dir_path, Io.Dir.cwd(), dist_dir, g.io) catch |err| {
        Io.Dir.cwd().deleteTree(g.io, work_dir_path) catch {};
        try u.eprintln("failed to move files to target directory: {any}", .{err});
        return 1;
    };
